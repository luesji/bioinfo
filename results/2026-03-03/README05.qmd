...
- *Entry*: elementos funcionales predichos sobre las proteínas (dominio, familia, región funcional...). Se pueden 
...
*reviewed* (Swiss-Prot) (revisadas manualmente, más fiables) o *unreviewed* (TrEMBL) (anotación automática, hay muchas más).
...
str(proteinas)
```
[La función str te da la estructura de un objeto.]{style="color:orange"}
...
[En URL: "api/entry/interpro"-> quiero las entries de interpro // "api/entry/pfam"-> quiero las entries de pfam. Pero como hay miles, solo nos da los 20 primeros para no saturar, y ofrece una nueva URL que aparece como next para cargar otros 20 (como pasar de página).]{style="color:orange"}
...
##del comando de abajo: definimos pfam01 para ver las entries de la base de datos pfam
## class nos indica que clase es el elemento pfam01-> es una lista
##length cuantas categorias hay-> 4
##names como se llaman esas categorias-> count, next, previus y results
##count me dice el número de resultados que hay
##next es para ver los 20 siguientes
##previus es para ver los 20 anteriores, pero al ser la primera vez, no tenemos previus
##results son los resultados, y al añadir metadata nos indica información de resultados (transmembrana etc)
```{r}
...
[Se define pfam01 para ver las entries de la base de datos pfam.]{style="color:orange"}

[class nos indica qué clase es el elemento]{style="color:orange"} 

[length nos dice cuántas categorías hay en total, que en este objeto son 4.]{style="color:orange"}

[names nos muestra cómo se llaman esas categorías, que son count, next, previous y results.]{style="color:orange"} 

[count nos dice el número total de resultados que hay en la consulta.]{style="color:orange"} 

[next sirve para ver los 20 resultados siguientes en la base de datos.]{style="color:orange"} 

[previous es para ver los 20 anteriores, pero al ser la primera vez, aparece vacío.]{style="color:orange"}

[results contiene los resultados propiamente dichos y, al añadir metadata, nos indica información detallada de los mismos.]{style="color:orange"}
...
[Se juntan las 20 primeras páginas con las siguientes 20 en una sola tabla (ejecutar pfam para verlo).]{style="color:orange"}
...
```{r}
taxot <- fromJSON(paste0(API,'taxonomy'))[['taxa']][['uniprot']]
taxo <- fromJSON(paste0(API,'taxonomy/protein'))[['taxa']][['uniprot']]
taxorest <- taxot - taxo
taxorest
```

```{r}
taxot1 <- fromJSON(paste0(API,'taxonomy'))
taxo1 <- fromJSON(paste0(API,'taxonomy/protein'))
taxorest1 <- taxot1 - taxo1

```

[Taxot es el total; taxo son las que tienen proteína asignada. Como quiero las que no tienen proteína asignada, será restar el total (Taxot) menos las que tienen proteína asignada (Taxo). Además, como no se pueden restar tablas se tiene que añadir [[taxa]][[uniprot]] para "entrar en la caja" que es donde estan los valores numéricos. Si se hace sin [[taxa]][[uniprot]] sale error no numérico (me pasó al principio), porque estás restando tablas. La solución sale al ejecutar taxorest: 1387422 taxones sin proteína asignada.]{style="color:green"}
...
```{r}
verteb <- fromJSON(paste0(API, 'entry/pfam/taxonomy/uniprot/7742'))
verteb
```
...
[RESPUESTA: 15 proteínas]{style="color:green"}

...
###instalar paquetes con el comando install.packages("")
# Objetivos
...
tu sesión de R.
:::
##la URL en el ejemplo de entries era "registros"
```{r}
...
```
## str(proteinas)  te da la estructura de un objeto
```{r}
str(proteinas)
```
...
##en los URL: api/entry/interpro-> quiero las entries de interpro
## api/entry/pfam-> quiero los entries de pfam
##pero como hay miles, solo nos da los 20 primeros para no saturar, y ofrece una nueva URL
## que aparece como next-> para cargar otros 20 (como pasar de página en amazon)
...
##esto nos muestra los siguientes 20 de la lista "next"
##y lo de kableExtra es para obtener una tabla "bonita"
```{r}
library('kableExtra')
...
```
##lo de arriba junta las 20 primeras y las 20 segundas en una sola tabla, ejecutar pfam para verlo
...
##protein porque es proteina y reviewed porque corresponde a swiss-prot
:::
```{r}
...
##sirve para ver cuantas entries hay dentro de la categoria proteínas
...
##taxot es el total, taxo son las q tienen proteina asignada, como quiero las que NO, resto el total menos las q SI tienen proteina asignada (taxot-taxo)
##además, como no puedo restar tablas se añade [[taxa]][[uniprot]] para "entrar en la caja" que es donde estan los valores numericos
##si lo haces sin [[]][[]] te sale error no numérico, porque estas restando tablas
##SOLUCIÓN: 1387422 taxones sin proteína asignada##
:::
```{r}
